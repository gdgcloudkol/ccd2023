...
import { AccomodationRequired, OptionsType, SpeakerDataModel, TalkData, TechType } from '../assets/models/speaker/datatype';
import { ApiSpeaker, ApiSpeakerList, ApiTalk, ApiTechnologies } from '../services/speaker.service';
...
const CFS = () => {
  ...
  const [speakerData, setSpeakerDta] = useState<number[]>([]);
  ...
  useEffect(() => {
    const checkForSpeaker = async () => {
      ...
      if (result.status === 200 && result.data.length > 0) {
        setSpeaker(true)
        setSpeakerDta(pkVal(result?.data))
      }
    ...
    const fetchTechnology = async () => {
      ...
      if (result.status === 200) {
        let options: { label: string, value: number }[] = result?.data?.results?.map((i: any) => {
          return {
            label: i.name,
            value: i.id
          }
      ...
      <div className="min-h-full flex">
        <div className="flex-1 flex flex-col justify-center py-12 px-4 sm:px-6 lg:flex-none lg:px-20 xl:px-24">
          {isSpeaker ?
            ...
            </div> : isLoading ? <GoogleDotsLoader /> : <div className=' mx-auto w-full max-w-sm lg:w-96 dark:text-black text-white'>
              <div>
                <h2 className="mt-6 text-2xl text-gray-900 dark:text-gray-100 tracking-tight">
              ...
              <div className="mt-8">
                <div className="mt-6">
                  <form method="POST" className="space-y-6" id="cfs">
                    ...
                    <div className="">
                      <label
                        htmlFor="topic_of_experties"
                        className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1"
                      >
                        Topic of Expertise
                      </label>
                      <Select closeMenuOnSelect={false} isMulti options={technologies as any} name="topic_of_experties"
                        onChange={(e: MultiValue<OptionsType>) =>
                          handleMultiChange(e)
                        }
                      />
