...
import {
  ...
  TalkData,
  TechTypeData
} from '../assets/models/speaker/datatype';
...
const CFS = () => {
  ...
  const [has_spoken_previously, setHasSpokenPreviously] =
    ...
    Promise.all([
      ...
      if (tech.status === 200) {
        let options: { label: string; value: number }[] =
          tech?.data?.results?.map((i: TechTypeData) => {
  ...
  async function handleSubmit(e: any) {
    ...
    if (isSpeaker) {
      ...
      if (previous_talk_links === '') {
        setFieldErrors({
          previous_talk_links:
            'Please provide comma separated previous talks link'
        });
        setSpinnerLoading(false);
        return;
      }
      ...
      if (result?.status > 199 && result?.status < 301) {
        ...
        setSpinnerLoading(false);
        setFieldErrors({
          error: 'There is a problem creating speaker profile'
        });
      }
    }
  }
  ...
  const handleMultiChange = (
    e: MultiValue<MultiSelectOptionsType>,
    type: 's' | 't'
  ) => {
    if (type === 's')
  ...
  return (
    <>
      <Helmet>
        <title>
          Call for Speakers | Google Cloud Community Days Kolkata 2023
        </title>
        <meta
          name="description"
          content="Submit your talk for Google Cloud Community Days Kolkata 2023"
        />
      </Helmet>
      {isLoading ? (
        <GoogleDotsLoader />
        ...
        <div className={`max-w-7xl mx-auto ${cfsState === 'cfsc' && !isSubmitted ? 'lg:h-72' : ''}`} data-aos="fade-up">
          <div className="min-h-full w-full flex">
            <div className="flex-1 flex flex-col justify-center m-5 ">
              {isSpeaker ? (
                isSubmitted ? (
                  <div className="flex m-5 items-center flex-col justify-center">
                    <div className="w-full flex flex-col items-center justify-center">
                      ...
                      <h4 className="py-2 text-lg lg:text-xl text-center text-google-yellow tracking-tight">
                        Note: Talks can be editted only upto 72 hours post
                        submission
                      </h4>
                      <CFSSettings
                        technologiesList={technologiesList}
                        talkData={submittedTalks}
                        ...
                        <form method="POST" className="space-y-6" id="cfs">
                          ...
                          <div>
                            ...
                            <Select
                              ...
                              isMulti
                              options={
                                technologiesList as MultiSelectOptionsType[]
                              }
                              name="technologies"
                              onChange={(
                                e: MultiValue<MultiSelectOptionsType>
                              ) => handleMultiChange(e, 't')}
                            />
                      ...
                      <form className="space-y-6" id="cfs">
                        ...
                        <div>
                          ...
                          <Select
                            ...
                            onChange={(e: MultiValue<MultiSelectOptionsType>) =>
                              handleMultiChange(e, 's')
                            }
                          />
